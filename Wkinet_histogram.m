...
xlims = [1.e0 1*1.e9];
ylims = [1.e-13 1.e0];
xticks = [1.e0  1.e3  1.e6  1.e9 ]% 1.e8 1.e9];
...
if strcmp(fitBodyType, 'maxw')
    ...
    maxwfitBody = fit(xWf(idxRegbody1)/maxwFitCorrCoeff,hWf(idxRegbody1)*maxwFitCorrCoeff,maxw,'StartPoint',100)
...
ph_i = plot(x,y,'o-.','Color',[0.2941, 0.4447, 0.6494]);
...
%}
x = xWf(idxftail1-2:end);
y = exp(bfTail) * x.^(zfTail);
ph_ft = plot(x, y, '--', 'LineWidth',2, 'Color',[0 0.75 0]);

if p_law
x2 = xWf(idxftail3-2:end);
y2 = exp(bfTail2) * x2.^(zfTail2);
ph_ft2 = plot(x2, y2, '--', 'LineWidth',2, 'Color',[0.25 0.5 0.94]);
end
...
if strcmp(fitBodyType, 'maxw')
    ...
    fitBodyLgnd = ['Maxwellian fit, kT = ', num2str(aBody*0.001,4), ' KeV'];
end
...
if isempty(fitBodyType)
    ...
    legend([ph_i, ph_f,ph_ft2, ph_ft], 'Location', 'NorthEast',...
     ...
     if p_law
    legend([ph_i, ph_f, ph_fb,ph_ft2, ph_ft], 'Location', 'NorthEast',...
           {'Initial distribution',...
            ['Distribution after t = ',num2str(tM(tidx),2), ' sec'],...
            fitBodyLgnd,...
...
if tfin>0.9
 ...
 elseif imodel == 2 
    ...
    if imodel == 1
     temp=['png_files_upper\f8a_small.png']; saveas(gca,temp);
    temp1=['eps_files_upper\f8a_small.eps']; saveas(gca,temp1); 
    temp2=['fig_files_upper\f8a_small.fig']; saveas(gca,temp2); 
    
 elseif imodel == 12
          temp=['new_png\f8a_small.png']; saveas(gca,temp);
    temp1=['new_eps\f8a_small.eps']; saveas(gca,temp1); 
    temp2=['new_fig\f8a_small.fig']; saveas(gca,temp2); 
 elseif imodel == 2 
     
      temp=['new_png_rec\f8a_small P=',num2str(P_reconnect),'.png']; saveas(gca,temp);
    temp1=['new_eps_rec\f8a_small P=',num2str(P_reconnect),'.eps']; saveas(gca,temp1); 
    temp2=['new_fig_rec\f8a_small P=',num2str(P_reconnect),'.fig']; saveas(gca,temp2); 
...
%{
if Non_reconn_UCS
  temp=['tests_nonRec\f12a P(rec)=',num2str(P_reconnect),'L=',num2str(L),'.png']; saveas(gca,temp);
else
    
temp=['tests\f12a P(ASs)=',num2str(PdW),'L=',num2str(L),'.png']; saveas(gca,temp);
